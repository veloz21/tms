...
export class MenuHorizontalComponent implements OnInit, AfterViewInit {
	...
	offcanvasOptions: OffcanvasOptions = {
		overlay: true,
		baseClass: 'b404-header-menu-wrapper',
	...
	getItemCssClasses(item) {
		...
		if (objectPath.get(item, 'resizer')) {
			classes += ' b404-menu__item--resize';
		}
	...
	getItemMenuSubmenuClass(item) {
		...
		if (alignment) {
			classes += ' b404-menu__submenu--' + alignment;
		}

		const type = objectPath.get(item, 'type') || 'classic';
		if (type === 'classic') {
			classes += ' b404-menu__submenu--classic';
		}
		if (type === 'tabs') {
			classes += ' b404-menu__submenu--tabs';
		}
		if (type === 'mega') {
			if (objectPath.get(item, 'width')) {
				classes += ' b404-menu__submenu--fixed';
			}
		}

		if (objectPath.get(item, 'pull')) {
			classes += ' b404-menu__submenu--pull';
		}
