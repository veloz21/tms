...
@Component({
  selector: 'b404-portlet-header',
  styleUrls: ['portlet-header.component.scss'],
  template: `
		<div class="b404-portlet__head-label" [hidden]="noTitle">
			<span class="b404-portlet__head-icon" #refIcon [hidden]="hideIcon || !icon">
			...
			<h3 *ngIf="title" class="b404-portlet__head-title" [innerHTML]="title"></h3>
		</div>
		<div class="b404-portlet__head-toolbar" #refTools [hidden]="hideTools">
...
export class PortletHeaderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @HostBinding('class') classes = 'b404-portlet__head';
  ...
  updateStickyPosition() {
    if (this.sticky) {
      Promise.resolve(null).then(() => {
        ...
        const headerElement = document.querySelector('.b404-header') as HTMLElement;
        const subheaderElement = document.querySelector('.b404-subheader') as HTMLElement;
        const headerMobileElement = document.querySelector('.b404-header-mobile') as HTMLElement;
        ...
        if (headerElement != null) {
          ...
          if (window.getComputedStyle(headerElement).height === '0px') {
            ...
            // desktop header
            if (document.body.classList.contains('b404-header--minimize-topbar')) {
              // hardcoded minimized header height
              height = 60;
            } else {
              // normal fixed header
              if (document.body.classList.contains('b404-header--fixed')) {
                height += headerElement.offsetHeight;
              }
              if (document.body.classList.contains('b404-subheader--fixed')) {
                height += subheaderElement ? subheaderElement.offsetHeight : 0;
              }
            }
          }
        }
