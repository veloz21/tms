...
export class UsersService {
  ...
  findOne(_id: Types.ObjectId, options: HttpOptions): Promise<UserDocument> {
  ...
  update(_id: Types.ObjectId, updateUserDto: UpdateUserDto, options: HttpOptions): Promise<UserDocument> {
  ...
  partialUpdate(_id: Types.ObjectId, partialUser: Partial<UserDocument>, options: HttpOptions): Promise<UserDocument> {
    return this.userModel.update({ _id, company: options.company }, { $set: partialUser }, { new: true, session: options.session }).exec();
  }

  remove(_id: Types.ObjectId, options: HttpOptions): Promise<UserDocument> {
